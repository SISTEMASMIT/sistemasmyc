...
$(document).ready(function () {
    ...
    fecha.oneDate("#f1");
    $('#agencias option[value="Todas"]').remove();
...
$(document).on('click', '#consultas_lista_por_agencias', function() {
    boton=true;
    traer_data();
});
...
async function traer_data(){
    ...
    let parametros = ["agencias","cifras","loteria","f1"];
    let extras = {};
    //parametros,  extras, moneda, comando/id 
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"consultas_lista_por_agencias"));
    }
    let tabla_info = {"stack":window[moneda_actual],
        "parametros":parametros,
        "moneda":moneda_actual,
        "titulo":"Lista por Agencias",
