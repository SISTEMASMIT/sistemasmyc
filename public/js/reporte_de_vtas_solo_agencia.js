...
$(document).on('click', '#repote_de_vtas_solo_agencias', function() {
    boton=true;
    traer_data();
});
...
async function traer_data(){
    ...
    let parametros = ["receptor","grupo_agencias","agencia","f1f2","loteria","ventas_mostrar","agrupar_reporte"];
    let extras = {};
    //parametros,  extras, moneda, comando/id 
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"repote_de_vtas_solo_agencias"));
    }
    let tabla_info = {"stack":window[moneda_actual],
        "parametros":parametros,
        "moneda":moneda_actual,
        "titulo":"Reporte de Ventas Agencia",
