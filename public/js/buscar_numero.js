...
$(document).on('click', '#buscar_numero', function() {
    if($('#numero').val()!=''){
        boton=true;
        traer_data();
    }else{
...
async function traer_data(){
    ...
    let parametros = ["receptor","loteria_mix","numero","f1"];
    let extras = {};
    //parametros,  extras, moneda, comando/id 
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"buscar_numero"));
    }
