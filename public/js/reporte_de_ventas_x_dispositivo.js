...
$(document).on('click', '#resumen_de_ventas_x_dispositivo', function() {
    boton=true;
    traer_data();
});
...
async function traer_data(){
    ...
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"resumen_de_ventas_x_dispositivo"));
    }
    let tabla_info = {"stack":window[moneda_actual],
        "parametros":parametros,
        "moneda":moneda_actual,
        "titulo":"Reporte de Ventas x Dispositivo",
