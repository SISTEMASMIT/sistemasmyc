...
$(document).on('click', '#resumen_de_ventas_x_ticket', function() {
    boton=true;
    traer_data();
});
...
async function traer_data(){
    ...
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"resumen_de_ventas_x_ticket"));
    }
    let tabla_info = {"stack":window[moneda_actual],
        "parametros":parametros,
        "moneda":moneda_actual,
        "titulo":"Resumen Ventas x Ticket",
...
$(document).on('dblclick', 'td', async function () {
    ...
    if($(base).children().length>2){
        prev_data.push(row.find("td").eq(7).text());
