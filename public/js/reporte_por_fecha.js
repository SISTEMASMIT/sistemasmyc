...
$(document).on('click', '#reporte_por_fecha', async function() {
...
//      Para sacar un rango de fechas se usa el mismo identificador lo que cambia es el metodo
   ...
   data = {"receptor":receptores,"loteria":loterias,"f1":f1,"f2":f2,"producto_loteria":productos,"grupo_agencias":grupos,"agencia":agencias,"comando":"reporte_por_fecha"};
   var info =  await ajax_peticion("/query/standard_query", {'data': JSON.stringify(data)}, "POST");
   ...
   let labels = {"receptor":receptores,"loteria_mix":loterias,"f1":f1V,"f2":f2V,"producto_loteria":productos,"grupo_agencias":grupos,"agencia":agencias,"comando":"reporte_por_fecha"};
   crear_tabla(info.data,"#tabla1","#thead1","#tbody1",isdclick,dclick,isrclick,invisibles,sumatorias,labels,'Reporte Por Fecha');
    ...
    if(have_dclick.length>0){$("#load").addClass('spinner');}
       ...
       if(iscorrect){
           ...
           keys.forEach((key,index)=>{
            //aqui lo envio manual porque tenemos una unica emergente
            if(key=="c4"){
                key="c2";
            }else
            if(key=="c2"){
