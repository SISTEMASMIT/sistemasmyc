...
$(document).on('click', '#ventas_global_sistemas_consolodidados', function() {
    boton=true;
    traer_data();
});
...
async function traer_data(){
    ...
    let parametros = ["receptor","f1f2"];
    let extras = {};
    //parametros,  extras, moneda, comando/id 
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"ventas_global_sistemas_consolodidados"));
    }
    let tabla_info = {"stack":window[moneda_actual],
        "parametros":parametros,
        "moneda":moneda_actual,
        "titulo":"Ventas Global Sistemas Consolidades",
