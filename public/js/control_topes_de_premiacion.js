...
$(document).on('click', '#control_de_topes_premiacion', function() {
    boton=true;
    traer_data();
});
...
async function traer_data(){
    ...
    let parametros = ["receptor","grupo_agencias","loteria_mix"];
    let extras = {"tope_premia":"agencias"};
    //parametros,  extras, moneda, comando/id 
    if(window[moneda_actual].size()<1 || boton==true){
        window[moneda_actual].push(await gestor.consulta(parametros,extras, moneda_actual,"control_de_topes_premiacion"));
    }
    let tabla_info = {"stack":window[moneda_actual],
        "parametros":parametros,
        "moneda":moneda_actual,
        "titulo":"Topes Premiación",
